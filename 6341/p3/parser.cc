...
int eval(struct node* input, struct node* output) {
    ...
    } else if (input->terminal && input->token->type == LITERAL_ATOM_ID && input->token->literal == "NIL") {
        output->nil = true;
        output->terminal = true;
        output->left_child = NULL;
        output->right_child = NULL;
    ...
    } else if (input->terminal && input->token && input->token->type == NUMERIC_ATOM_ID && input->token->numeral) {
        output->nil = false;
        output->terminal = true;
        output->left_child = NULL;
        output->right_child = NULL;
            ...
            if (listLength == 3 && (input->left_child->token->literal == "PLUS" ||
                ...
                if (s1Ret == EXIT_FAILURE) {
                    ...
                    if (input->left_child->token->literal == "ATOM") {
                        ...
                        if (s1->terminal && s1->token && s1->token->type == LITERAL_ATOM_ID &&
    ...
    do {
        ...
        if (parseExpr(expr_root) == EXIT_SUCCESS) {
            ...
            if (eval(expr_root, output_root) == EXIT_SUCCESS) {
                // Pretty print output_root
                printExpr(output_root);
            } else {
                cout << "EXIT FAILUURE" << endl;
            }
        } else {
            break;
        }
